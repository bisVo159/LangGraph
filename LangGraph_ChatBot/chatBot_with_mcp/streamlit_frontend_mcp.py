...
if "db_initialized" not in st.session_state:
    asyncio.run(langgraph_mcp_backend.init_db())
    st.session_state.db_initialized = True

def create_new_chat():
    new_id = generate_thread_id()
    asyncio.run(langgraph_mcp_backend.save_thread_db(new_id, "New Chat"))
...
def load_chat_history(thread_id):
    ...
    messages = asyncio.run(langgraph_mcp_backend.get_history(thread_id))
...
if "threads" not in st.session_state:
    all_threads = asyncio.run(langgraph_mcp_backend.get_all_threads_db())
    if all_threads:
...
with st.sidebar:
    ...
    with col2:
        if st.button("🗑️ Delete Chat"):
            ...
            asyncio.run(langgraph_mcp_backend.delete_thread_db(tid))
    ...
    if st.button("🧹 Clear All Chats"):
        asyncio.run(langgraph_mcp_backend.clear_db())
        st.session_state.threads = {}
        create_new_chat()
        st.rerun()
    ...
    if st.session_state.rename_mode:
        ...
        if c3.button("✅ Save"):
            asyncio.run(langgraph_mcp_backend.save_thread_db(st.session_state.active_thread, new_name))
    ...
    thread_ids = list(st.session_state.threads.keys())[::-1] # Reverse order
    ...
    with st.chat_message("assistant"):
        ...
        async def run_chat():
            ...
            if st.session_state.threads[st.session_state.active_thread] == "New Chat":
                ...
                await langgraph_mcp_backend.save_thread_db(st.session_state.active_thread, "New Chat")
            
            async for chunk, metadata in langgraph_mcp_backend.stream_chat(st.session_state.active_thread, user_input):
