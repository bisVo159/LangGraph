...
def process_pdf(thread_id: str, file_data: bytes):
    """
    Receives raw file bytes, saves to a temp file, 
    creates a FAISS index specific to the thread_id, and cleans up.
    """
    ...
    try:
        ...
        _VECTOR_STORES[str(thread_id)] = FAISS.from_documents(splits, embeddings)

        return f"Successfully processed {len(splits)} chunks for this chat."
...
def retrieve_knowledge_base(query: str, config: RunnableConfig) -> str:
    """
    Retrieve relevant information from the uploaded PDF/document for information. 
    Use this tool when the user asks questions about a specific file they uploaded.
    """
    global _VECTOR_STORES
    thread_id = config["configurable"].get("thread_id")
    if not thread_id or thread_id not in _VECTOR_STORES:
        return "No document found for this chat. Please ask the user to upload a PDF first."
    
    vector_store = _VECTOR_STORES[str(thread_id)]

    try:
        results = vector_store.similarity_search(query, k=3)
        context = "\n\n".join([doc.page_content for doc in results])
        return f"Found in uploaded document:\n{context}"
    except Exception as e:
...
async def get_tools_and_builder():
    ...
    if _CACHED_TOOLS is None:
        try:
            ...
            mcp_tools = []
        _CACHED_TOOLS = [search_tool, get_stock_price, calculator, retrieve_knowledge_base] + mcp_tools
